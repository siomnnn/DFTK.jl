...
"""
function diagonalize_all_kblocks(eigensolver, ham::Hamiltonian{B}, nev_per_kpoint::Int;
                                 ψguess=nothing,
                                 prec_type=PreconditionerTPA, interpolate_kpoints=true,
                                 tol=1e-6, miniter=1, maxiter=100, n_conv_check=nothing) where {B <: PlaneWaveBasis}
...
function diagonalize_all_kblocks(eigensolver, ham::Hamiltonian{B}, nev_per_kpoint::Int;
                                 ψguess=nothing,
                                 prec_type=PreconditionerNone, interpolate_kpoints=true,
                                 tol=1e-6, miniter=1, maxiter=100, n_conv_check=nothing) where {B <: FiniteElementBasis}
    kpoints = ham.basis.kpoints
    results = Vector{Any}(undef, length(kpoints))

    for (ik, kpt) in enumerate(kpoints)
        n_dofs = get_n_free_dofs(ham.basis, :ψ)
        if n_dofs < nev_per_kpoint
            error("The size of the finite element basis is $n_dofs, and you are asking for " *
                  "$nev_per_kpoint eigenvalues. Use a finer mesh or increase polynomial degree.")
        end
        # Get ψguessk
        if !isnothing(ψguess)
            if n_dofs != size(ψguess[ik], 1)
                error("Mismatch in dimension between guess ($(size(ψguess[ik], 1)) and " *
                      "Hamiltonian ($n_dofs)")
            end
            nev_guess = size(ψguess[ik], 2)
            ...
            elseif nev_guess == nev_per_kpoint
                ...
                X0 = similar(ψguess[ik], n_dofs, nev_per_kpoint)
                X0[:, 1:nev_guess] = ψguess[ik]
                X0[:, nev_guess+1:end] = randn(eltype(X0), n_dofs, nev_per_kpoint - nev_guess)
                ψguessk = ortho_qr(X0)
            end
        elseif interpolate_kpoints && ik > 1
            ψguessk = results[ik - 1].X     # reuse previous solution as guess. BCs are applied on the fly anyways
        else
            ψguessk = random_orbitals(ham.basis, kpt, nev_per_kpoint)
        end
        @assert size(ψguessk) == (n_dofs, nev_per_kpoint)
